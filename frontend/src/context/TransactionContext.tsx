...
export const TransactionProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const uploadCSV = async (file: File) => {
    try {
      ...
      if (parsed.length === 0) {
        throw new Error(
          'CSV was uploaded but no transactions could be parsed. Please verify headers like Date, Receiver Name, Amount, Mode of Transaction, and Transaction Type (debit/credit).',
        );
      }
